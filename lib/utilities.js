...
utilities.adminSignupValidator = (data, expectedData)=>{
    ...
    if(dataKeys.length === expectedData.length){
       for(let  datum of dataKeys){
            if(expectedData.includes(datum)){
                if(datum === "username" && (typeof data[datum] !== "string" || !usernameRegex.test(data[datum].trim()))){
                    return {
                        isValid: false,
                        errorField: datum,
                        msg: `${datum} should be a string and it should not be empty`
                    }
                }

                if(datum === "email" && (typeof data[datum] !== "string" || !emailRegex.test(data[datum].trim()))){
                    return {
                        isValid: false,
                        errorField: datum,
                        msg: `${datum} should be a valid email and it should not be empty`
                    }
                }
    
                if(datum === "password" && (typeof data[datum] !== "string" || !passwordRegex.test(data[datum].trim()))){
                    return {
                        isValid: false,
                        errorField: datum,
                        msg: `${datum} should be a string and it should not be empty`
                ...
                return {
                    isValid: false,
                    msg: `${datum} is not required for admin signup`
                }
            }
        }
        ...
        return {
            isValid: false,
            msg: `incomplete data or excess data detected`
        }
    }
...
utilities.payloadValidator = (data, expectedData)=>{
    const dataKeys = Object.keys(data)

    if(dataKeys.length == expectedData.length){
        for(let datum of dataKeys){
            if(!data[datum] || !expectedData.includes(datum) || typeof data[datum] != "string" || data[datum].length < 1){
                return {
                    isValid: false,
                    msg: `Invalid ${datum} please make sure your data is in string format and not empty`
                }
            }
